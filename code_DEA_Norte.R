...
bp2019 <- ggplot(dados_full, aes(x = sigla, y = eficiencia, fill = sigla)) +
  ...
  stat_summary(fun = function(x) quantile(x, probs = 0.25), geom = "text", 
               aes(label = round(..y.., 1)),
               hjust = -1, vjust = 1.2, color = "black", size = 2) +
...
dados_full_mapa %>%
  ...
  labs(fill     = "Eficiência",
       title    = "DEA: Eficiência dos Municípios do Norte - 2019") +
  theme_void()
...
dados_full_mapa %>%
  ...
  scale_fill_viridis_d(option = "mako", na.value = "grey90") +
  labs(fill     = "Eficiência",
       title    = "DEA: Eficiência dos Municípios do Norte - 2021") +
  theme_void()
...
dados_full_mapa %>%
  ...
  scale_fill_viridis_d(option = "mako", na.value = "grey90") +
  labs(fill     = "Eficiência",
       title    = "DEA: Eficiência dos Municípios do Norte - 2023") +
  theme_void()
