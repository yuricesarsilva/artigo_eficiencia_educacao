...
# Produto: Rodando o DEA Nordeste
################################################################################
...
bp2019 <- ggplot(dados_full, aes(x = sigla, y = eficiencia, fill = sigla)) +
  ...
  stat_summary(fun = function(x) quantile(x, probs = 0.25), geom = "text", 
               aes(label = round(..y.., 1)),
               hjust = -0.6, vjust = 1.2, color = "black", size = 2) +
  stat_summary(fun = function(x) quantile(x, probs = 0.75), geom = "text", 
               aes(label = round(..y.., 1)),
               hjust = -0.6, vjust = -0.5, color = "black", size = 2) +
...
dados_full_mapa %>%
  ...
  labs(fill     = "Eficiência",
       title    = "DEA: Eficiência dos Municípios do Nordeste - 2019") +
  theme_void()
...
dados_full_mapa %>%
  ...
  scale_fill_viridis_d(option = "mako", na.value = "grey90") +
  labs(fill     = "Eficiência",
       title    = "DEA: Eficiência dos Municípios do Nordeste - 2021") +
  theme_void()
...
dados_full_mapa %>%
  ...
  scale_fill_viridis_d(option = "mako", na.value = "grey90") +
  labs(fill     = "Eficiência",
       title    = "DEA: Eficiência dos Municípios do Nordeste - 2023") +
  theme_void()
...
write.xlsx(escores2019, "results/escores_nordeste_2019.xlsx")
write.xlsx(benchmarks2019, "results/benchmarks_nordeste_2019.xlsx")

write.xlsx(escores2021, "results/escores_nordeste_2021.xlsx")
write.xlsx(benchmarks2021, "results/benchmarks_nordeste_2021.xlsx")

write.xlsx(escores2023, "results/escores_nordeste_2023.xlsx")
write.xlsx(benchmarks2023, "results/benchmarks_nordeste_2023.xlsx")
